...
export async function getProfilePreferences(accessToken, fetchImpl = fetch) {
  const rows = await restRequest(
    'profiles?select=target_titles,title_aliases,target_locations,remote_preference,salary_min,industries,seniority_targets,company_sizes,work_authorization,excluded_companies,application_profile&limit=1',
    accessToken,
    {},
    fetchImpl,
  );
